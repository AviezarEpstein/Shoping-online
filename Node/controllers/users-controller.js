...
router.get('/', getAlluserDetails); // public to all kind of users

async function getAlluserDetails(request, response, next){
  try{
    let token = request.headers.authorization;
    let decoded = jwt_decode(token);
    let userId = decoded.id;
    let userData = await usersLogic.getAlluserDetails(userId);
